...
public class SegmentTree3 {
	...
	public static int value = 1000000001;
	...
	public static int initTree(int start, int end, int node) {
		...
		int mid = (start + end) / 2;
		return tree[node] = Math.min(initTree(start, mid, node * 2), initTree(mid + 1, end, node * 2 + 1));
	}
	...
	public static int calc(int start, int end, int node, int left, int right) {
		...
		if (left <= start && right >= end)
			return tree[node];
		int mid = (start + end) / 2;
		return Math.min(calc(start, mid, node * 2, left, right), calc(mid + 1, end, node * 2 + 1, left, right));
	}
